...
import { Sparkles, Users, Megaphone, Home } from 'lucide-react'

type Item = { id: string; label: string; sub?: string; href: string }

const QUICK_LINKS: Item[] = [
  { id: 'go-home',      label: 'Ir a Inicio',     href: '/' },
  { id: 'go-leads',     label: 'Ir a Leads',      href: '/leads' },
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const [clientsRes, dealsRes] = await Promise.all([
        sb.from('clients').select('id,legal_name,commercial_name,client_type').is('deleted_at', null).ilike('legal_name', `%${q}%`).limit(5),
        sb.from('deals').select('id,title,stage').is('deleted_at', null).neq('stage', 'ganado').neq('stage', 'perdido').ilike('title', `%${q}%`).limit(5),
      ])
      if (cancelled) return
      ...
      const ds = (dealsRes.data ?? []).map((d: { id: string; title: string; stage: string }) => ({ id: `d-${d.id}`, label: d.title, sub: `Lead · ${d.stage}`, href: `/leads?lead=${d.id}` }))
      setResults([...cs, ...ds])
    })()
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[12vh] px-4" onClick={() => onOpenChange(false)}>
      ...
      <div className="relative w-full max-w-xl" onClick={(e) => e.stopPropagation()}>
        <Command label="Command palette" className="rounded-xl border border-border bg-bg-surface shadow-2xl overflow-hidden">
          <Command.Input
            value={query}
            onValueChange={setQuery}
            placeholder="Buscar leads o clientes…"
            className="w-full px-4 py-3.5 bg-transparent border-b border-border text-sm text-slate-100 placeholder:text-slate-600 focus:outline-none"
          />
          <Command.List className="max-h-[360px] overflow-y-auto py-2">
            ...
            <Command.Group heading="Navegación" className="px-2 mt-1 text-[10px] uppercase tracking-wider text-slate-600">
              {QUICK_LINKS.map((l) => {
...
function iconFor(href: string) {
  if (href.startsWith('/leads')) return Sparkles
